...
bool Population::spawn(Genome *g,int size) {
	...
	//TODO: if all the inputs are disconnected to the outputs,
	//we should already add some random connections:
	//if(NEAT::all_inputs_disconnected)
	//	epoch(0);
...
// NEW
bool Population::print_champ_to_file(char *filename) {

  std::vector<Organism*>::iterator curorg;
  Organism* bestOrg = *organisms.begin();

  //Step through the organisms in the population to find the best one
  for(curorg = organisms.begin(); curorg != organisms.end(); ++curorg) {
  	if((*curorg)->fitness > bestOrg->fitness)
  		bestOrg = *curorg;
  }

  return bestOrg->print_to_file(filename);
}
...
bool Population::epoch(int generation) {
	...
	// Way to automatically set the compatibility threshold for speciation (see FAQ):
