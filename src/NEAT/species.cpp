...
bool Species::print_to_file(std::ostream &outFile) {
	...
	//outFile<<std::endl<<"/* Species #"<<id<<" : (Size "<<organisms.size()<<") (AF "<<ave_fitness<<") (Age "<<age<<")  */"<<std::endl<<std::endl;
	char tempbuf[1024];
	sprintf(tempbuf,"/* Species #%d : (Size %lu) (AF %f) (Age %d)  */\n\n", id, organisms.size(), average_est, age);
...
void Species::adjust_fitness() {
	...
	//NEW: If there is at least one fitness which is negative,
	//readjust the fitness of every organism such that they are all positive.
	//If all the value are positive, then min is equal to 0.
	double min = 0.0;
	for(curorg = organisms.begin(); curorg != organisms.end(); ++curorg) {
		if( ((*curorg)->fitness) < min)
			min = (*curorg)->fitness;
	}
	...
	for(curorg=organisms.begin();curorg!=organisms.end();++curorg) {
		...
		//OLD: Do not allow negative fitness
		//if (((*curorg)->fitness)<0.0) (*curorg)->fitness=0.0001;

		//NEW: Allow negative fitness by readjusting those values.
		((*curorg)->fitness) -= min;
	...
	//Update: It's not correct to add 1.0 in the floor(...), try with NEAT::survival_thresh=0.2,
	// and you will obtain that num_parents=3 when it should be 2...
	...
	if(num_parents == 0) num_parents++;

	//Mark for death those who are ranked too low to be parents
	curorg = organisms.begin();
	(*curorg)->champion = true;  //Mark the champ as such, it will be saved unchanged in the next generation (elitism).
...
bool Species::reproduce(int generation, Population *pop,std::vector<Species*> &sorted_species) {
	...
	// NOTE: At this point, only the valid parents are inside the species, all the other organisms have been deleted
	// from the species (see code in Population::epoch).
	...
	// NEW: allow Elitism (use NEAT::elitism_percentage)
	int NbElitist = (int) floor( NEAT::elitism_percentage * expected_offspring );
	std::vector<Organism*>::iterator curChamp = organisms.begin();
	//std::cout << "NbElitist: " << NbElitist << std::endl;
		...
		for (count=0; count<expected_offspring; count++) {
			...
			if ((thechamp->super_champ_offspring) > 0) {
				std::cout << "we have a super champ!!" << std::endl;
				mom=thechamp;
			...
			//If we have a Species champion, just clone it
				...
				std::cout << "we have a champ!!" << std::endl;

				// NEW: Elitism
				if(NEAT::elitism_percentage == 0.0) {
					mom = thechamp; //Mom is the champ
					champ_done = true;
				} else {
					mom = *curChamp;
					curChamp++;
					if(count == (NbElitist-1))
						champ_done = true;
				}
				...
				std::cout << "Mutation only!!" << std::endl;
					...
					//std::cout << "Based on parent number " << orgnum+1 << std::endl;
					//std::cout << "Species size: " << organisms.size() << std::endl;
					...
					// Add sensor: immediately connects a randomly chosen sensor which was previously disconnected to all the outputs at once.
					// See FAQ: This is useful if we start NEAT with some disconnected (inputs) sensors.
					// By default, mutate_add_sensor_prob = 0.0
					else if (randfloat() < NEAT::mutate_add_sensor_prob) {
						//std::cout << "mutate add sensor" << std::endl;
						new_genome->mutate_add_sensor(pop->innovations, pop->cur_innov_num);
						mut_struct_baby = true;
					}
					...
					(((dad->gnome)->compatibility(mom->gnome))==0.0))
				{
					...
					// By default, mutate_add_sensor_prob = 0.0
						...
						new_genome->mutate_add_sensor(pop->innovations, pop->cur_innov_num);
						//std::cout << "mutate add sensor: " << new_genome << std::endl;
						mut_struct_baby = true;
					}
					else {
